...
#if defined(__AVR_ATmega32U4__) || defined(__SAM3X8E__)															// Serail 0 on leonardo is used for USB, thus we need to use Serail1
...
void DynamixelClass::begin(HardwareSerial &HWserial, long baud, unsigned char D_Pin){	
...
#if defined(__AVR_ATmega32U4__) || defined(__SAM3X8E__)
	Serial1.begin(baud); // Set up Serial for a specified Serial object
    _serial = &Serial1;					
#else 
	Serial.begin(baud);   // Set up Serial for all others (Uno, etc)
    _serial = &Serial;				
...
void DynamixelClass::end(){
	...
	#if defined(__AVR_ATmega32U4__) || defined(__MK20DX128__) || defined(__AVR_ATmega2560__) || defined(__SAM3X8E__)
    	Serial1.end();
	#else
    	Serial.end();
	#endif
